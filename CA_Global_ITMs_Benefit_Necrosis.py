...
def get_global_cost(lattice, params):
    ...
    # print (global_cost_apoptotic, global_cost_necrotic)
    # if ITMs_remaining > 0:
...
def compute_b_necrosis(params, lattice):
    count_apoptotic, count_necrotic = count_neutrophils(lattice, params)
    params['b_necrosis'] = params['alpha'] * (params['ITMs'] ** params['k'] - count_necrotic * params['m']
                                              - count_apoptotic * params['n'])
    return params
...
def compute_cost_remaining_ITMs(params, lattice):
    count_apoptotic, count_necrotic = count_neutrophils(lattice, params)
    apoptotic_cost = params['alpha'] * (params['ITMs'] ** params['k'] - count_necrotic * params['m']
                                              - (count_apoptotic + 1) * params['n'])
    necrotic_cost = params['alpha'] * (params['ITMs'] ** params['k'] - (count_necrotic + 1) * params['m']
                                        - count_apoptotic * params['n'])

    # print (apoptotic_cost, necrotic_cost)
    return apoptotic_cost*7.9, necrotic_cost*7.9


def get_initial_local_ITMs(params, lattice):
    count_apoptotic, count_necrotic = count_neutrophils(lattice, params)
    return params['ITMs_initial'] - params['m'] * count_necrotic - params['n'] * count_apoptotic
...
def process_lattice(params, index):
    ...
    for i, j in xy_pairs:
        if lattice[i, j] == params['_activated']:
            params['ITMs'] = get_initial_local_ITMs(params, lattice)
            params = compute_b_necrosis(params, lattice)
            ...
            if all_activated:
                ...
                necrotic_local_pay_off = get_local_payoff_necrotic(lattice, i, j, params)
                apoptotic_local_pay_off = get_local_payoff_apoptotic(lattice, i, j, params)
...
if __name__ == "__main__":
    project_dir = 'C:/Users/user/Google Drive/Alva Modeling Immune System/Innate Immunity/Journal Papers/Neutrophils ' \
                  'Game/results/Cellular Automata/Global ITMs/Necrosis Payoff/'
    ...
    if not os.path.exists(project_dir + params['experiment']):
        os.makedirs(project_dir + params['experiment'])

    res_df = df.join(res_df)
    Date_Now = pd.datetime.now().strftime("%Y-%m-%d_%H-%M")
    res_df.to_csv(project_dir + params['experiment'] + '/' + Date_Now + '_res_' + str(int(params['ITMs'])) + '.csv')