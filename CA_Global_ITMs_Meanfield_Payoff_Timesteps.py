...
def fill_lattice_necrotic(lattice, params, index):
    ...
    for i in range(n):
        for j in range(m):
            if lattice[i, j] == params['_empty'] and count < params['necrotic_baseline']:
                lattice[i, j] = params['_necrotic']
                count += 1
    return lattice
...
def compute_global_cost(lattice, params):
    count_apoptotic, count_necrotic = count_neutrophils(lattice, params)
    global_cost = params['alpha'] * (params['ITMs'] ** params['k'] - count_necrotic * params['m']
                                     - count_apoptotic * params['n'])
    if global_cost >= 0:
...
def process_lattice(params, trial, index):
    count = 0
    lattice = create_lattice(params['x'], params['y'])
    new_lattice = fill_lattice(lattice, params, trial)
    lattice = fill_lattice_necrotic(new_lattice, params, trial)
    xy_pairs = generate_random_pairs(params['x'], params['y'], trial)
    results = {'parameter': [],
               'ITMs': [],
               'timestep': [],
               'apoptotic': [],
    ...
    for i, j in xy_pairs:
        if lattice[i, j] == params['_activated']:
            ...
            if all_activated:
                ...
                necrotic_local_pay_off = get_local_payoff_necrotic(lattice, i, j, params) - params['mult'] * np.exp(
                    global_necrotic_cost)
                apoptotic_local_pay_off = get_local_payoff_apoptotic(lattice, i, j, params) - params['mult'] * np.exp(
                    global_apoptotic_cost)
            ...
            results['parameter'].append(index)
            results['ITMs'].append(params['ITMs'])
            results['timestep'].append(count)
            results['apoptotic'].append(apoptotic)
            results['necrotic'].append(necrotic)
...
def sweep_params(params, ITMs, trial, index):
    start = timer()
    print(str(trial) + ' started ')
    params['ITMs'] = ITMs
    results = process_lattice(params, trial, index)
    end = timer()
    print(str(trial) + ' finished, execution time: ' + str(end - start))
    return results


def do_parallel(params, index):
    pairs = list(itertools.product(params['ITMs_list'], range(params['trials'])))
...
def pick_parameters(parameter_file, params, project_directory):
    df = pd.read_csv(parameter_file)
    index_list = random.sample(range(len(df.index)), params['count'])
    df = df.iloc[index_list]
    df.to_csv(project_directory + 'parameters_timesteps.csv')


if __name__ == "__main__":
    project_dir = 'C:/Users/user/Google Drive/Alva Modeling Immune System/Innate Immunity/Journal Papers/Neutrophils ' \
                  'Game/results/Cellular Automata/Global ITMs/Time Steps/'
    param_file = 'C:/Users/user/Google Drive/Alva Modeling Immune System/Innate Immunity/Journal Papers/Neutrophils ' \
                 'Game/results/Cellular Automata/Global ITMs/Time Steps/parameters_timesteps.csv'
    ...
    for index, row in df.iterrows():
        params['b_apoptosis'] = row['b_apoptosis']
        params['c_necrosis'] = row['c_necrosis']
        params['m'] = row['m']
        params['n'] = row['n']
        params['alpha'] = row['alpha']

        results = do_parallel(params, index)
        results_list.append(results)

    res_df = pd.concat(list(itertools.chain.from_iterable(results_list)))

    if not os.path.exists(project_dir):
        os.makedirs(project_dir)

    Date_Now = pd.datetime.now().strftime("%Y-%m-%d_%H-%M")
    res_df.to_csv(project_dir + '/' + Date_Now + '_res.csv')