...
def compute_cost_remaining_ITMs(lattice, params, i, j):
    ...
    count_apoptotic, count_necrotic, count_activated = count_neighbors(params, lattice, i, j)
    initial_total_local_ITMs = initial_local_ITMs * (((params['moore'] * 2) + 1) ** 2)
    global_local_cost_apoptosis = params['alpha'] * (initial_total_local_ITMs ** params['k']
                                                     - count_necrotic * params['m'] - (count_apoptotic +1) * params['n'])
    global_local_cost_necrosis = params['alpha'] * (
                initial_total_local_ITMs ** params['k'] - (count_necrotic + 1) * params['m']
                - count_apoptotic * params['n'])
...
def process_lattice(params, index):
    ...
    for i, j in xy_pairs:
        if lattice[i,j] == params['_activated']:
            ...
            cost_remaining_apoptosis, cost_remaining_necrosis = compute_cost_remaining_ITMs(lattice, params, i, j)
            ...
            if all_activated:
                ...
                necrotic_local_pay_off = necrotic_local_pay_off - len_moore * cost_remaining_necrosis
                apoptotic_local_pay_off = apoptotic_local_pay_off - len_moore * cost_remaining_apoptosis
...
if __name__ == "__main__":
    project_dir = 'C:/Users/user/Google Drive/Alva Modeling Immune System/Innate Immunity/Journal Papers/Neutrophils ' \
                  'Game/results/Cellular Automata/Local ITMs/ITMs Payoff/'
